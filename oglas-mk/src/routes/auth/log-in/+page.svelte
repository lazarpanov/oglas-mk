...
<div class="flex h-full w-full items-start justify-center px-4 pt-12">
	<form class="flex flex-col space-y-4 rounded-xl p-8 shadow-2xl w-[400px]" on:submit|preventDefault={login}
	>
		...
		<div class="flex flex-col justify-center gap-4">
			<label class="label" style="padding-top: 10px;">
				<span>Email</span>
				<!-- (input here) -->
				<input class="input px-2 py-1" title="email" type="text" placeholder="Email" required
				bind:value={email}/>
			</label>
			<label class="label">
				<span>Password</span>
				<!-- (input here) -->
				<input class="input px-2 py-1" title="password" type="text" placeholder="Password" required
				bind:value={password}/>
			</label>
